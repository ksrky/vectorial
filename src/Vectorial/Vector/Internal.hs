...
newtype V a = V [(CC, a)]
    deriving Consumable via [(CC, a)]
    deriving Dupable via [(CC, a)]
    deriving Movable via [(CC, a)]
      ...
      where
        ...
        addV c1 x ((c2, y) : bys) =
            case (dup2 x, dup2 y) of
                ((x', x''), (y', y'')) ->
                    case x' == y' of
                        True  -> consume x'' `lseq` ((c1 + c2, y'') : bys)
...
instance (Eq a, Dupable a) => Module CC (V a) where
    (*>) c (V xs) = case move c of
        Ur c' -> V $ map (\(c1, x) -> (c' * c1, x)) xs
...
instance RMonad V where
    ...
    return x = V [(1, x)]
    (>>=) :: forall a b. (Restrict V b) => V a %1 -> (a %1 -> V b) %1 -> V b
